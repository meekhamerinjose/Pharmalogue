...
public class PillBox {
    ...
    public long addDrugEntry(Context c, Drug drug) {
        db = new DbHelper(c);
        long drugId = db.createDrugDbEntry(drug);
        drug.setDrugId(drugId);
        db.close();
        return drugId;
    }

    public void addDrugs(Context c) {
        db = new DbHelper(c);
        db.truncateDrugsTable();
        for (Drug d : DrugDatabase.drugArrayList) {
            long drugId = db.createDrugDbEntry(d);
            d.setDrugId(drugId);
        }
        db.close();
    }
    ...
    public List<Drug> getAllDrugs(Context c) {
        db = new DbHelper(c);
        List<Drug> drugs = db.getDrugs();
        db.close();
        return drugs;
    }
}
